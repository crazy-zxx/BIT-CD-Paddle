...
def get_parser():
    ...
    parser.add_argument('--weight_path', type=str, default=r"output/BIT/best_model/model.pdparams",
                        help='weight path')
...
if __name__ == "__main__":
    ...
    if os.path.exists(os.path.abspath(args.A)) and os.path.exists(os.path.abspath(args.B)):

        imgs = os.listdir(args.A)

        for fn in imgs:
            image = {'image_t1': os.path.join(os.path.abspath(args.A), fn),
                     'image_t2': os.path.join(os.path.abspath(args.B), fn)}
            ...
            cv2.imwrite(os.path.join(os.path.abspath(args.pre), fn), vis)
            print(fn + ' finish!')
